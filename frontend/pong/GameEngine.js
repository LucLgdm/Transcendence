...
export class GameEngine {
    constructor(canvasId, settings = DEFAULT_SETTINGS) {
        this.keys = {};
            ...
            if (!this.matchEnded) {
                if (this.ball) {
                    if (this.settings.mode === 'ai' || this.settings.mode === 'aix2') {
                        ...
                        if (this.keys['o'] || this.keys['arrowup'])
                            this.Rplayer.moveUp();
                        if (this.keys['l'] || this.keys['arrowdown'])
                            this.Rplayer.moveDown();
                    }
        ...
        this.Lplayer = new Paddle(-GAME_CONFIG.PADDLE.POS_X, GAME_CONFIG.PADDLE.POS_Y, GAME_CONFIG.PADDLE.POS_Z, GAME_CONFIG.PADDLE.COLOR, settings.paddleSpeedMultiplier);
        this.Rplayer = new Paddle(GAME_CONFIG.PADDLE.POS_X, GAME_CONFIG.PADDLE.POS_Y, GAME_CONFIG.PADDLE.POS_Z, GAME_CONFIG.PADDLE.COLOR, settings.paddleSpeedMultiplier);
    ...
    triggerPowerUp() {
        if (this.powerUpMesh || this.isResetting || this.matchEnded)
            return;
    ...
    checkCollisions() {
        if (this.matchEnded || this.isResetting)
            return;
        ...
        if (Math.abs(ballPos.x) > GAME_CONFIG.ARENA.SCORE_X) {
            ...
            if (this.score.left >= winScore || this.score.right >= winScore) {
                const winner = this.score.left >= winScore ? 'left' : 'right';
...
export function disposePongIfAny() {
    if (pongInstance) {
...
export function initPong() {
    ...
    setPongCanvasPlaying(false);
    if (pongMenuListenersBound)
        return;
    pongMenuListenersBound = true;
    startBtn.addEventListener('click', () => {
        const mode = document.getElementById('config-mode').value;
        const themeColor = parseInt(document.getElementById('config-theme').value, 16);
        const speed = parseFloat(document.getElementById('config-speed').value);
        const powerups = document.getElementById('config-powerups').checked;
        const aiDifficulty = document.getElementById('config-ai-difficulty').value;
