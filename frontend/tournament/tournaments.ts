...
function alertApiError(res: Response, body: unknown): void {
...
function phaseLabel(round: number, capacity: number): string {
	const numRounds = Math.round(Math.log2(capacity));
	if (capacity === 8) {
		const keys = ["tournament-phase-qf", "tournament-phase-sf", "tournament-phase-f"] as const;
		return t(keys[round] ?? "tournament-phase-generic");
	}
	if (capacity === 4) {
		const keys = ["tournament-phase-sf", "tournament-phase-f"] as const;
		return t(keys[round] ?? "tournament-phase-generic");
	}
	return `${t("tournament-phase-generic")} ${round + 1}/${numRounds}`;
}

function userNameMap(participants: ParticipantRow[]): Map<number, string> {
...
function nameOrId(id: number | null, names: Map<number, string>): string {
	if (id === null) return "—";
...
async function loadList(): Promise<void> {
	...
	root.innerHTML = rows
		...
		.join("");
	root.querySelectorAll<HTMLButtonElement>(".tournament-list-item").forEach((btn) => {
		btn.addEventListener("click", () => {
			const id = Number(btn.dataset.tid);
			if (Number.isFinite(id)) void showDetail(id);
		});
	});
}
...
function statusLabel(status: string): string {
	if (status === "registration") return t("tournament-status-registration");
...
async function showDetail(id: number): Promise<void> {
	...
	const canRegister =
		tournament.status === "registration" && me !== null && !imRegistered && participants.length < tournament.capacity;
	...
	document.getElementById("tournament-btn-delete")?.addEventListener("click", async () => {
		if (!confirm(t("tournament-delete-confirm"))) return;
	...
	detail.querySelectorAll<HTMLButtonElement>(".tournament-win-btn").forEach((btn) => {
		btn.addEventListener("click", async () => {
			const mid = Number(btn.dataset.mid);
			const wid = Number(btn.dataset.wid);
			if (!Number.isFinite(mid) || !Number.isFinite(wid)) return;
			...
			const j = (await r.json().catch(() => ({}))) as { tournamentCompleted?: boolean };
			if (j.tournamentCompleted) alert(t("tournament-completed-ok"));
			await loadList();
			await showDetail(id);
		});
	});
...
export async function refreshTournamentsView(): Promise<void> {
	const section = document.getElementById("view-tournaments");
	if (section) applyTranslations(section);
	await loadList();
	if (selectedTournamentId !== null) await showDetail(selectedTournamentId);
}

export function initTournaments(): void {
	const form = document.getElementById("tournament-create-form") as HTMLFormElement | null;
